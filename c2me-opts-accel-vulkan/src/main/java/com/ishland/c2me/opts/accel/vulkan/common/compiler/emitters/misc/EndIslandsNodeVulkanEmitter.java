...
public class EndIslandsNodeVulkanEmitter implements VulkanCEmitter<EndIslandsNode> {
    public static final EndIslandsNodeVulkanEmitter INSTANCE = new EndIslandsNodeVulkanEmitter();

    private EndIslandsNodeVulkanEmitter() {
    }

    @Override
    public String doGen(EndIslandsNode node, VulkanGenFunctionContext context, String storeTo) {
        ...
        return "global const uint32_t * const permutation = ptr_shift_global(ctx.const_data, " + offset + ");\n" +
                storeTo + " = ((double) math_end_islands_sample_global(permutation, ctx.x / 8, ctx.z / 8) - 8.0) / 128.0;\n";
    }
}

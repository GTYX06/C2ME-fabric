...
package com.ishland.c2me.opts.accel.vulkan.mixin.chunksystem_integration;

import com.ishland.c2me.opts.accel.vulkan.common.chunksystem_integration.BatchingBiomeNoiseStatus;
...
public abstract class MixinVanillaWorldGenerationDelegate {
    ...
    private KeyStatusPair<ChunkPos, ChunkState, ChunkLoadingContext>[] appendBatchingDependency(KeyStatusPair<ChunkPos, ChunkState, ChunkLoadingContext>[] original, ItemHolder<ChunkPos, ChunkState, ChunkLoadingContext, ?> holder) {
        if (this.status == ChunkStatus.BIOMES) {
            ...
            if (!VulkanServerBatchedBiomeNoiseContext.isAligned(pos.x(), pos.z())) { // aligned pos implicitly depends on itself
                KeyStatusPair<ChunkPos, ChunkState, ChunkLoadingContext>[] newArray = Arrays.copyOf(original, original.length + 1);
                newArray[original.length] = new KeyStatusPair<>(new ChunkPos(pos.x() & ~VulkanServerBatchedBiomeNoiseContext.BATCH_MASK, pos.z() & ~VulkanServerBatchedBiomeNoiseContext.BATCH_MASK), BatchingBiomeNoiseStatus.getInstance());
                return newArray;
            }
        }

...
import com.ishland.c2me.opts.dfc.common.gen.vulkan.VulkanGenContext;
...
//    public static final Object MARKER_structureWeightSamplerTable = new Object();
    ...
    public static GeneratedVulkanSource compile(NoiseRouter noiseRouter, GenerationShapeConfig generationShapeConfig, Reference2ReferenceMap<DensityFunction, OptoPasses.AstPair> optoCache, DensityFunction finalFinalDensity, BiomeSource biomeSource) {
        ...
        GeneratedVulkanSource original = context.build();
        String name = "DfcCompiled_" + original.getOrdinal();
        Path path = GenDumper.dumpGLSL(name, original.getGeneratedSource().getBytes(StandardCharsets.UTF_8));
    ...
    public static class FunctionContextImpl implements VulkanGenFunctionContext {
        ...
        @Override
        public VulkanGenContext getGlobalContext() {
        ...
        private String newVar0(AstNode node) {
            ...
            String generated = VulkanGenRegistry.doGen(node, this, varName);
            this.pendingBody
        ...
        public String getDelegateVar(ValuesMethodDef target, AstNode.ReturnType returnType) {
            ...
            if (target.isConst()) {
                return switch (target) {
                    case ValuesMethodDefF32 f32 -> VulkanGen.literal(f32.constValue());
                    case ValuesMethodDefF64 f64 -> VulkanGen.literal(f64.constValue());
        ...
        @Override
        public VulkanGenFunctionContext fork() {
    ...
    public static class ContextImpl implements VulkanGenContext {
...
//            this.allocGlobalDynamicData(MARKER_structureWeightSamplerTable);
        ...
        public ValuesMethodDef newDispatcher(AstNode node, String id, AstNode.ReturnType returnType) {
            ...
            for (VulkanGenFunctionContext.FunctionVariant variant : VulkanGenFunctionContext.FunctionVariant.values()) {
            ...
            this.pendingSource
                    ...
                    .append("    ").append("    ").append("return ").append(id).append(VulkanGenFunctionContext.FunctionVariant.FULLY_CACHED.suffix).append("(ctx);\n")
                    .append("    ").append("} else if (ctx.rw_data &&(ctx.sample_flags & MASK_enableFlatCache) == MASK_enableFlatCache) {\n")
                    .append("    ").append("    ").append("return ").append(id).append(VulkanGenFunctionContext.FunctionVariant.FLATCACHE_ONLY.suffix).append("(ctx);\n")
                    .append("    ").append("} else {\n")
                    .append("    ").append("    ").append("return ").append(id).append(VulkanGenFunctionContext.FunctionVariant.UNCACHED.suffix).append("(ctx);\n")
                    .append("    ").append("}\n")
                    .append("}\n");
        ...
        @Override
        public ValuesMethodDef newMethod(AstNode node, VulkanGenFunctionContext.FunctionVariant variant, AstNode.ReturnType returnType) {
            validateNodeType(node, returnType);
            if (node instanceof ConstantNodeLike constantNodeLike) {
                return constantNodeLike.getDef();
            } else {
        ...
        @Override
        public ValuesMethodDefF64 newMethodF64(AstNode node, VulkanGenFunctionContext.FunctionVariant variant) {
            return (ValuesMethodDefF64) this.newMethod(node, variant, AstNode.ReturnType.F64);
        }

        @Override
        public ValuesMethodDefF32 newMethodF32(AstNode node, VulkanGenFunctionContext.FunctionVariant variant) {
            return (ValuesMethodDefF32) this.newMethod(node, variant, AstNode.ReturnType.F32);
        }

        public String newMethodUnoptimized(AstNode node, VulkanGenFunctionContext.FunctionVariant variant) {
        ...
        public int allocGlobalConstDataObject(Object obj) {
            byte[] bytes = VulkanGen.bytesObject(obj);
        ...
        public void genNoiseKernels() {
...

            {
                ...
                for (Object2IntMap.Entry<CacheLikeNode> entry : caches.object2IntEntrySet()) {
                    ...
                    validateNodeType(node, AstNode.ReturnType.F64);
                    String delegateName = this.newMethodUnoptimized(node.getDelegate(), VulkanGenFunctionContext.FunctionVariant.UNCACHED);
...

            {
                ...
                for (Object2IntMap.Entry<CacheLikeNode> entry : caches.object2IntEntrySet()) {
                    ...
                    int i = entry.getIntValue();
                    validateNodeType(node, AstNode.ReturnType.F64);
                    String delegateName = this.newMethodUnoptimized(node.getDelegate(), VulkanGenFunctionContext.FunctionVariant.FULLY_CACHED_EXCEPT_CACHE2D);
...

            {
                ...
                for (Object2IntMap.Entry<CacheLikeNode> entry : caches.object2IntEntrySet()) {
                    ...
                    int i = entry.getIntValue();
                    validateNodeType(node, AstNode.ReturnType.F64);
                    String delegateName = this.newMethodUnoptimized(node.getDelegate(), VulkanGenFunctionContext.FunctionVariant.FLATCACHE_ONLY);
                    Assertions.assertTrue(delegateName != null);
...
//                            .append("    ").append(name).append("(const_data, rw_data, extra_out);\n");
        ...
        public GeneratedVulkanSource build() {
        ...
        private record FunctionKey(AstNode node, VulkanGenFunctionContext.FunctionVariant variant) {
        }

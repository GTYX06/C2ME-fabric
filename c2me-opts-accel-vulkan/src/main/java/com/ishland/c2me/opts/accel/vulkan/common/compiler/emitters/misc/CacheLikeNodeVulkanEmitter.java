...
public class CacheLikeNodeVulkanEmitter implements VulkanCEmitter<CacheLikeNode> {
    public static final CacheLikeNodeVulkanEmitter INSTANCE = new CacheLikeNodeVulkanEmitter();

    private CacheLikeNodeVulkanEmitter() {
    }

    @Override
    public String doGen(CacheLikeNode node, VulkanGenFunctionContext context, String storeTo) {
        ...
        return switch (wrapping.type()) {
            ...
            case INTERPOLATED -> {
                if (context.getVariant().enableAllCache) {
                    int offset = context.getGlobalContext().getGlobalDynamicDataOffset(VulkanGen.MARKER_cacheLike_interpolator);
            ...
            case FLAT_CACHE -> {
                if (context.getVariant().enableFlatCache) {
                    int offset = context.getGlobalContext().getGlobalDynamicDataOffset(VulkanGen.MARKER_cacheLike_flatCache);
            ...
            case CACHE2D -> {
                if (context.getVariant().useCache2D()) {
                    int offset = context.getGlobalContext().getGlobalDynamicDataOffset(VulkanGen.MARKER_cacheLike_cache2d);

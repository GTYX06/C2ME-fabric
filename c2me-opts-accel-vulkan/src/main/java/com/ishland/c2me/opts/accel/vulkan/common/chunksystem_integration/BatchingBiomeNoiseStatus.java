...
package com.ishland.c2me.opts.accel.vulkan.common.chunksystem_integration;

import com.ishland.c2me.base.mixin.access.IThreadedAnvilChunkStorage;
import com.ishland.c2me.opts.accel.vulkan.common.ducks.TACSExtension;
import com.ishland.c2me.opts.accel.vulkan.common.gen.VulkanServerBatchedBiomeNoiseContext;
...
public class BatchingBiomeNoiseStatus extends NewChunkStatus {
    ...
    public Completable upgradeToThis(ChunkLoadingContext context, Cancellable cancellable) {
        ...
        if (!VulkanServerBatchedBiomeNoiseContext.isAligned(pos.x(), pos.z()) || startingChunk.getStatus().isAtLeast(ChunkStatus.NOISE)) {
        ...
        VulkanServerWorldContext clContext = ((TACSExtension) context.tacs()).c2me$getCLContext();
        if (clContext == null) {
            return Completable.complete();
        }
        ...
        BoundedRegionArray<ProtoChunk> boundedRegionArray = new BoundedRegionArray<>(
                pos.x(),
                pos.z(),
                VulkanServerBatchedBiomeNoiseContext.BATCH_SIZE,
                VulkanServerBatchedBiomeNoiseContext.BATCH_SIZE,
        ...
        for (int dx = 0; dx < VulkanServerBatchedBiomeNoiseContext.BATCH_SIZE; dx++) {
            for (int dz = 0; dz < VulkanServerBatchedBiomeNoiseContext.BATCH_SIZE; dz++) {
        ...
        return Completable.defer(() -> {
            return Completable.fromCompletionStage(VanillaWorldGenerationDelegate.runTaskWithLockArea(
                    ...
                    () -> {
                        VulkanServerBatchedBiomeNoiseContext batchedBiomeNoiseContext = new VulkanServerBatchedBiomeNoiseContext(pos, clContext, noiseChunkGenerator, chunkGenerationContext.world().getChunkManager().getNoiseConfig());
    ...
    public KeyStatusPair<ChunkPos, ChunkState, ChunkLoadingContext>[] getDependencies(ItemHolder<ChunkPos, ChunkState, ChunkLoadingContext, ?> holder) {
        ...
        if (VulkanServerBatchedBiomeNoiseContext.isAligned(pos.x(), pos.z()) && !chunk.getStatus().isAtLeast(ChunkStatus.NOISE)) {
    ...
    private KeyStatusPair<ChunkPos, ChunkState, ChunkLoadingContext> @NotNull [] getGenDeps(ChunkPos pos) {
        KeyStatusPair<ChunkPos, ChunkState, ChunkLoadingContext>[] deps = new KeyStatusPair[(VulkanServerBatchedBiomeNoiseContext.BATCH_SIZE + 16) * (VulkanServerBatchedBiomeNoiseContext.BATCH_SIZE + 16) - 1];
        int index = 0;
        for (int dx = -8; dx < VulkanServerBatchedBiomeNoiseContext.BATCH_SIZE + 8; dx++) { // padding for structure_starts
            for (int dz = -8; dz < VulkanServerBatchedBiomeNoiseContext.BATCH_SIZE + 8; dz++) {
                if (dx == 0 && dz == 0) continue; // skip self
                if (dx >= 0 && dx < VulkanServerBatchedBiomeNoiseContext.BATCH_SIZE && dz >= 0 && dz < VulkanServerBatchedBiomeNoiseContext.BATCH_SIZE) {

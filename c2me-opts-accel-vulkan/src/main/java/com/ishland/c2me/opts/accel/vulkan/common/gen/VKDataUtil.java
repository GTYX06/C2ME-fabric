...
import com.ishland.c2me.opts.accel.vulkan.common.util.VulkanStructs;
...
public class VKDataUtil {
    ...
    public static ByteBuffer worldgen_data_root$createForFlatCacheOnly(ChunkPos base, int chunkSize, GeneratedVulkanSource generatedCLSource, double[] flatCachePrefilled, boolean extendByOne) {
...
//        int localSize = 0;
        ...
        int currentTail = roundUp(VulkanStructs.GLOBAL_OFFSET_TABLE_START + generatedCLSource.getGlobalDynamicDataOffsets().size() * Integer.BYTES, 32);
        for (Reference2IntMap.Entry<Object> entry : generatedCLSource.getGlobalDynamicDataOffsets().reference2IntEntrySet()) {
            ...
            if (key == VulkanGen.MARKER_localOffsetTable) {
            ...
            } else if (key == VulkanGen.MARKER_cacheLike_cache2d) {
                allocatedOffsets[index] = new OffsetAndData(0, null); // no interpolator
            } else {
        ...
        VulkanStructs.worldgen_params$startBiomeX.set(segment, 0L, BiomeCoords.fromChunk(base.x()));
        VulkanStructs.worldgen_params$startBiomeZ.set(segment, 0L, BiomeCoords.fromChunk(base.z()));
        VulkanStructs.worldgen_params$sizeBiomeX.set(segment, 0L, (chunkSize * 4) - (extendByOne ? 0 : 1));
        VulkanStructs.worldgen_params$sizeBiomeZ.set(segment, 0L, (chunkSize * 4) - (extendByOne ? 0 : 1));
        VulkanStructs.worldgen_params$startCellX.set(segment, 0L, Integer.MAX_VALUE);
        VulkanStructs.worldgen_params$startCellY.set(segment, 0L, Integer.MAX_VALUE);
        VulkanStructs.worldgen_params$startCellZ.set(segment, 0L, Integer.MAX_VALUE);
        VulkanStructs.worldgen_params$sizeCellX.set(segment, 0L, 0);
        VulkanStructs.worldgen_params$sizeCellY.set(segment, 0L, 0);
        VulkanStructs.worldgen_params$sizeCellZ.set(segment, 0L, 0);
        VulkanStructs.worldgen_params$estimateSurfaceHeight_startBiomeX.set(segment, 0L, Integer.MAX_VALUE);
        VulkanStructs.worldgen_params$estimateSurfaceHeight_startBiomeZ.set(segment, 0L, Integer.MAX_VALUE);
        VulkanStructs.worldgen_params$estimateSurfaceHeight_sizeBiomeX.set(segment, 0L, 0);
        VulkanStructs.worldgen_params$estimateSurfaceHeight_sizeBiomeZ.set(segment, 0L, 0);
        VulkanStructs.worldgen_params$cache2d_startX.set(segment, 0L, Integer.MAX_VALUE);
        VulkanStructs.worldgen_params$cache2d_startZ.set(segment, 0L, Integer.MAX_VALUE);
        VulkanStructs.worldgen_params$cache2d_sizeX.set(segment, 0L, 0);
        VulkanStructs.worldgen_params$cache2d_sizeZ.set(segment, 0L, 0);
        VulkanStructs.worldgen_params$offset_estimateSurfaceHeight.set(segment, 0L, 0);
        VulkanStructs.worldgen_params$genConfig_defaultBlock.set(segment, 0L, 0);
        VulkanStructs.worldgen_params$genConfig_defaultFluid.set(segment, 0L, 0);
        VulkanStructs.worldgen_params$offset_aquifer.set(segment, 0L, 0);
        VulkanStructs.worldgen_params$offset_fluidLevelSampler.set(segment, 0L, 0);
        VulkanStructs.worldgen_params$offset_oreVeinRandom.set(segment, 0L, 0);
        ...
        for (int i = 0, allocatedOffsetsLength = allocatedOffsets.length; i < allocatedOffsetsLength; i++) {
            ...
            segment.set(ValueLayout.JAVA_INT, VulkanStructs.GLOBAL_OFFSET_TABLE_START + (long) i * Integer.BYTES, allocatedOffset.offset);
    ...
    public static ByteBuffer worldgen_data_root$createForArea(ChunkPos basePos, int horizontalChunkSize, BoundedRegionArray<ProtoChunk> regionArray, NoiseChunkGenerator generator,
                                                              ...
                                                              VKBlockStateMappings mappings, GeneratedVulkanSource generatedCLSource, Stage1Cache.AreaCacheEntry stage1Cache) {
        if (generatedCLSource == null) {
            throw new IllegalStateException("Generated CL source not found");
        }
        ...
        for (Reference2IntMap.Entry<Object> entry : generatedCLSource.getGlobalDynamicDataOffsets().reference2IntEntrySet()) {
            ...
            if (key == VulkanGen.MARKER_localOffsetTable) {
                ...
                try (Arena arena1 = Arena.ofConfined()) {
                    MemorySegment segment = VulkanStructs.sws_index$createForArea(arena1, basePos, regionArray, toChunkNoiseSampler, horizontalChunkSize, horizontalChunkSize);
...
//                    Assertions.assertTrue(stage1Cache.surfaceHeights().length == VulkanStructs.SIZE_estimateSurfaceHeight * VulkanStructs.SIZE_estimateSurfaceHeight);
                ...
                if (settings.hasAquifers()) {
                    ...
                    try (Arena arena1 = Arena.ofConfined()) {
                        ...
                        MemorySegment segment = VulkanStructs.aquifer_data$create(
                                arena1,
                ...
                byte[] data;
                try (Arena arena1 = Arena.ofConfined()) {
                    MemorySegment segment = VulkanStructs.fluidLevelSamplerCreate(
                            arena1,
                ...
                if (settings.oreVeins()) {
                    ...
                    try (Arena arena1 = Arena.ofConfined()) {
                        ...
                        VulkanStructs.setRandomState(segment, noiseConfig.getOreRandomDeriver());
                        data = new byte[(int) segment.byteSize()];
                        MemorySegment.copy(segment, ValueLayout.JAVA_BYTE, 0L, data, 0, data.length);
                    }
                    currentTail = offset + data.length;
        ...
        ByteBuffer byteBuffer = MemoryUtil.memAlloc(roundUp(currentTail, 32));
        MemorySegment segment = MemorySegment.ofBuffer(byteBuffer);
        VulkanStructs.worldgen_params$startBiomeX.set(segment, 0L, BiomeCoords.fromChunk(basePos.x()));
        VulkanStructs.worldgen_params$startBiomeZ.set(segment, 0L, BiomeCoords.fromChunk(basePos.z()));
        VulkanStructs.worldgen_params$sizeBiomeX.set(segment, 0L, horizontalChunkSize << 2);
        VulkanStructs.worldgen_params$sizeBiomeZ.set(segment, 0L, horizontalChunkSize << 2);
        VulkanStructs.worldgen_params$startCellX.set(segment, 0L, Math.floorDiv(basePos.getStartX(), generationShapeConfig.horizontalCellBlockCount()));
        VulkanStructs.worldgen_params$startCellY.set(segment, 0L, Math.floorDiv(generationShapeConfig.minimumY(), generationShapeConfig.verticalCellBlockCount()));
        VulkanStructs.worldgen_params$startCellZ.set(segment, 0L, Math.floorDiv(basePos.getStartZ(), generationShapeConfig.horizontalCellBlockCount()));
        VulkanStructs.worldgen_params$sizeCellX.set(segment, 0L, horizontalCellCount);
        VulkanStructs.worldgen_params$sizeCellY.set(segment, 0L, verticalCellCount);
        VulkanStructs.worldgen_params$sizeCellZ.set(segment, 0L, horizontalCellCount);
        VulkanStructs.worldgen_params$estimateSurfaceHeight_startBiomeX.set(segment, 0L, (basePos.x() - 4) << 2);
        VulkanStructs.worldgen_params$estimateSurfaceHeight_startBiomeZ.set(segment, 0L, (basePos.z() - 4) << 2);
        VulkanStructs.worldgen_params$estimateSurfaceHeight_sizeBiomeX.set(segment, 0L, 32 + (4 * horizontalChunkSize));
        VulkanStructs.worldgen_params$estimateSurfaceHeight_sizeBiomeZ.set(segment, 0L, 32 + (4 * horizontalChunkSize));
        VulkanStructs.worldgen_params$cache2d_startX.set(segment, 0L, basePos.getStartX());
        VulkanStructs.worldgen_params$cache2d_startZ.set(segment, 0L, basePos.getStartZ());
        VulkanStructs.worldgen_params$cache2d_sizeX.set(segment, 0L, (horizontalChunkSize * 16));
        VulkanStructs.worldgen_params$cache2d_sizeZ.set(segment, 0L, (horizontalChunkSize * 16));
        VulkanStructs.worldgen_params$offset_estimateSurfaceHeight.set(segment, 0L, allocatedOffsets[generatedCLSource.getGlobalDynamicDataOffsets().getInt(VulkanGen.MARKER_estimateSurfaceHeightCache)].offset);
        VulkanStructs.worldgen_params$genConfig_defaultBlock.set(segment, 0L, mappings.toId(settings.defaultBlock()));
        VulkanStructs.worldgen_params$genConfig_defaultFluid.set(segment, 0L, mappings.toId(settings.defaultFluid()));
        VulkanStructs.worldgen_params$offset_aquifer.set(segment, 0L, allocatedOffsets[generatedCLSource.getGlobalDynamicDataOffsets().getInt(VulkanGen.MARKER_aquifer)].offset);
        VulkanStructs.worldgen_params$offset_fluidLevelSampler.set(segment, 0L, allocatedOffsets[generatedCLSource.getGlobalDynamicDataOffsets().getInt(VulkanGen.MARKER_fluidLevelSampler)].offset);
        VulkanStructs.worldgen_params$offset_oreVeinRandom.set(segment, 0L, allocatedOffsets[generatedCLSource.getGlobalDynamicDataOffsets().getInt(VulkanGen.MARKER_oreVeinRandom)].offset);
    ...
    public static Reference2IntLinkedOpenHashMap<Object> transformGlobalDynamicDataOffsets(Reference2IntLinkedOpenHashMap<Object> globalDynamicDataOffsets) {
        ...
        while (iterator.hasNext()) {
            ...
            if (entry.getKey() instanceof InterpolatedNoiseSampler sampler) {
                ...
                byte[] bytes = VulkanGen.bytes(sampler);
                newMap.put(new ConstantBlob(bytes, 8), entry.getIntValue());
            } else {
